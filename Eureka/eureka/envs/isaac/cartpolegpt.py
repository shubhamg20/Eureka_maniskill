...
def compute_reward(dof_pos: torch.Tensor, dof_vel: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Assume dof_pos and dof_vel each have shape (num_envs, 2)
    pole_angle = dof_pos[:, 1]  # dof_pos[:, 1] is the pole angle in radians
    pole_velocity = dof_vel[:, 1]  # dof_vel[:, 1] is the pole angular velocity

    # Main reward for the pole being upright
    upright_reward_temp = 5.0
    upright_reward = torch.exp(-torch.abs(pole_angle) / upright_reward_temp)

    # Penalize high pole angular velocity
    velocity_penalty_temp = 1.0
    velocity_penalty = torch.exp(-torch.abs(pole_velocity) / velocity_penalty_temp)

    # Total reward is the sum of upright reward and velocity penalty
    total_reward = upright_reward + velocity_penalty

    # Return the total reward and the individual components for debugging or further analysis
    reward_dict = {
