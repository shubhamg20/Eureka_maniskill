...
class Humanoid(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.velocity, self.up_proj, self.heading_proj)
...
def compute_reward(
    ...
    """
    Reward function to make the humanoid run as fast as possible while maintaining proper posture and heading.
    
    Args:
    - velocity (torch.Tensor): Tensor of shape (batch_size, 3) containing the local velocity of the humanoid.
    - up_proj (torch.Tensor): Tensor of shape (batch_size,) projecting the up vector to the torso's up direction.
    - heading_proj (torch.Tensor): Tensor of shape (batch_size,) projecting the heading direction to the torso's heading direction.
    
    Returns:
    - Tuple[torch.Tensor, Dict[str, torch.Tensor]]: Total reward and dictionary of individual reward components.
    """
    
    # Extract forward velocity (assuming it's the x-coordinate)
    forward_velocity = velocity[:, 0]

    # Reward for running fast
    forward_vel_reward = forward_velocity

    # Reward for being upright (we want to incentivize this behavior)
    upright_reward = up_proj

    # Reward for heading in the desired direction
    heading_reward = heading_proj
    
    # Combine rewards
    reward = forward_vel_reward + 0.1 * upright_reward + 0.1 * heading_reward

    # Clip reward to make sure it's positive and encourage moving forward
    reward = torch.clamp(reward, min=0.0)
    
    # Returning individual components for logging or debugging
    reward_components = {
