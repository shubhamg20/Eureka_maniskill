...
class Ant(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.velocity, self.actions)
...
@torch.jit.script
def compute_reward(root_states: torch.Tensor, velocity: torch.Tensor, actions: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Extract the forward velocity component (assuming the forward direction is along the x-axis)
    forward_velocity = velocity[:, 0]

    # Define a temperature parameter for the exponential reward transformation
    forward_velocity_temp = 1.0

    # Correlate reward directly with forward velocity
    forward_velocity_reward = torch.exp(forward_velocity / forward_velocity_temp) - 1

    # Optionally, add a penalty for energy expended (sum of squared actions)
    energy_penalty = torch.sum(actions**2, dim=-1)
    energy_temp = 0.1

    # Transform the energy penalty exponentially
    energy_penalty_reward = -(torch.exp(energy_penalty / energy_temp) - 1)

    # Compute total reward
    total_reward = forward_velocity_reward + energy_penalty_reward
